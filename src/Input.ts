...
export class Input {
  ...
  constructor(stage: Stage) {
    ...
    window.addEventListener('keyup', (evt: KeyboardEvent) => { //TODO: Move to babylon style
      const key = evt.key.toLowerCase();
      const keyState = this.key[key];
      if (keyState) {
        if (keyState.pressed) { // TODO: Are these if statements actually needed? They're on the up/down events anyway
